...
class sanhigia_pedidos(flfacturac):
    ...
    def sanhigia_pedidos_pedidoListoPDA(self, model, oParam):
        ...
        if not curPedido.commitBuffer():
            return False
        if not qsatype.FLUtil.execSql(u"UPDATE lineaspedidosprov set sh_codtrabarecep = '{}' WHERE idpedido = {} AND sh_codtrabarecep IS NULL AND shcantalbaran > 0".format(codtrabajador, model.idpedido)):
