...
class sanhigia_pedidos(flfacturac):
    ...
    def sanhigia_pedidos_getFilters(self, model, name, template=None):
        ...
        if name == 'pedidosNoBorradores':
            ...
            q.setWhere(u"pda IN ('Pendiente', 'Listo PDA', 'Preparado', 'Albaranado', 'Parcial') AND servido IN ('No', 'Parcial') AND (sh_estadopago NOT IN ('Borrador', 'Borrador con promocion', 'Forma de pago bloqueada', 'Pagos pendientes', 'Pendiente validar PA', 'Devolucion por aprobar', 'Devolucion preparada', 'Devolucion aprobada') OR sh_estadopago is null)")
            if not q.exec_():
                return []
            # if q.size() > 100:
            #     return []
    ...
    def sanhigia_pedidos_generaPreparaciondepedidos(self, model, oParam):
        ...
        consulta_where = u"p.servido not like 'Sí' AND p.pda IN ('Pendiente', 'Listo PDA', 'Preparado') AND p.idpedido IN ({}) AND u.codubicacion >= '{}'  AND u.codubicacion <= '{}' AND (l.sh_preparacion is null OR l.sh_preparacion NOT LIKE 'En Curso') AND (p.sh_estadopago not in ('Borrador','Borrador con promocion', 'Forma de pago bloqueada', 'Pagos pendientes', 'Pendiente validar PA', 'Devolucion por aprobar', 'Devolucion preparada', 'Devolucion aprobada') OR p.sh_estadopago is null) GROUP BY p.idpedido".format(pedidoscli, ubicacionini, ubicacionfin)
        query = qsatype.FLSqlQuery()
        ...
        while query.next():
            ...
            num_lineas = qsatype.FLUtil.sqlSelect("pedidoscli p INNER JOIN lineaspedidoscli l on l.idpedido = p.idpedido", "COUNT(l.idlinea)", "p.idpedido = {0} AND  p.servido not like 'Sí' AND p.pda IN ('Pendiente', 'Listo PDA', 'Preparado') AND (l.sh_preparacion is null OR l.sh_preparacion NOT LIKE 'En Curso') AND (p.sh_estadopago not in ('Borrador','Borrador con promocion', 'Forma de pago bloqueada', 'Pagos pendientes', 'Pendiente validar PA', 'Devolucion por aprobar', 'Devolucion preparada', 'Devolucion aprobada') OR p.sh_estadopago is null)".format(query.value(0)), u"pedidoscli,lineaspedidoscli")
        ...
        if crearPreparacion:
            ...
            if not curPreparaciondepedidos.commitBuffer():
                return False
            print(str_idpedidos)
            if not qsatype.FLUtil.execSql(u"UPDATE lineaspedidoscli set sh_preparacion = 'En Curso', codpreparaciondepedido='{}' WHERE idlinea IN (select l.idlinea from pedidoscli p INNER JOIN lineaspedidoscli l on l.idpedido = p.idpedido INNER JOIN ubicacionesarticulo u ON l.referencia = u.referencia  WHERE p.servido not like 'Sí' AND p.pda IN ('Pendiente', 'Listo PDA', 'Preparado') AND p.idpedido IN ({}) AND u.codubicacion >= '{}' AND u.codubicacion <= '{}' AND (l.sh_preparacion is null OR l.sh_preparacion NOT LIKE 'En Curso') AND (p.sh_estadopago not in ('Borrador','Borrador con promocion', 'Forma de pago bloqueada', 'Pagos pendientes', 'Pendiente validar PA', 'Devolucion por aprobar', 'Devolucion preparada', 'Devolucion aprobada') OR p.sh_estadopago is null) AND (l.totalenalbaran < l.cantidad) AND NOT cerrada)".format(codpreparacion, str_idpedidos[:-1], ubicacionini, ubicacionfin)):
                return False
            resul["status"] = 1
    ...
    def sanhigia_pedidos_generaPreparaciondepedidosConStock(self, model, oParam):
        ...
        where_consulta = u"p.servido in ('No','Parcial') AND p.pda IN ('Pendiente', 'Listo PDA', 'Preparado', 'Parcial') AND u.codubicacion >= '{}' AND u.codubicacion <= '{}' AND (l.sh_preparacion is null OR l.sh_preparacion NOT LIKE 'En Curso') AND l.totalenalbaran <> l.cantidad AND s.cantidad > 0 AND (p.sh_estadopago not in ('Borrador','Borrador con promocion', 'Forma de pago bloqueada', 'Pagos pendientes', 'Pendiente validar PA', 'Devolucion por aprobar', 'Devolucion preparada', 'Devolucion aprobada') OR p.sh_estadopago is null)".format(ubicacionini, ubicacionfin)
